...
public class PvfFile : IDisposable
{
    ...
    */
    public PvfFile(string file)//本构造方法使用后需dispose释放内存，构造过程请放到线程中异步调用。
    {
        ...
        //分配内存
        var decryptedTree = new byte[header.dirTreeLength];
        //读取文件索引列表
        ...
        //模拟读取字节数组的指针
        var pos = 0;
        for (var i = 0; i < header.numFilesInDirTree; i++)
        {
            HeaderTreeNode item = new();
            //从pos位置开始读取HeaderTreeNode对象，返回值为指针应该偏移的字节数
            var a = item.ReadNodeFromBitArrStream(header, fs, decryptedTree, pos);
            if (a < 0)
    ...
    {
        //文件的第一位int是索引总数
        var count = BitConverter.ToInt32(unpackedFileByteArr, 0);

        for (var i = 0; i < count; i++)
        {
            //每次循环的第一个int是键开始的地址
            var startpos = BitConverter.ToInt32(unpackedFileByteArr, i * 4 + 4);
            //每次循环的第二个int是键结束的地址
            var endpos = BitConverter.ToInt32(unpackedFileByteArr, i * 4 + 8);
            //相减就是值的长度
            ...
            var pathName = Encoding.GetEncoding("BIG5").GetString(pathBytes).TrimEnd(new char[1]);
            //放到索引表中备用
    ...
    private void LoadNStringLst(byte[] unpackedFileByteArr)
    {
        ...
        //从第二位开始每次读取十个字节
        for (var i = 2; i < unpackedFileByteArr.Length; i += 10)
        {
            //如果是最后十个字节或者最后不满十个字节就不执行
            if (unpackedFileByteArr.Length - i >= 10)
            {
                //前6位干嘛的不知道，6-10位的int值是stringtable的键，取出来
                var k = stringBinMap[BitConverter.ToInt32(unpackedFileByteArr, i + 6)];
                ...
                {
                    //直接用编码取这个文件的内容
                    var full = Encoding.GetEncoding("BIG5").GetString(node.unpackedFileByteArr).TrimEnd(new char[1]);

                    //根据换行分割，逐行遍历
                    foreach (var line in full.Split(new char[2] { '\r', '\n' }))
                    {
                        //行包含符号'>'，如name_xxx>格斗家
                        if (line.Contains('>'))
                        {
                            //取键 name_xxx
                            var key = Util.FindTagKeyVal(line, "", ">");
                            //取值 格斗家
                            var val = Util.FindTagKeyVal(line, ">", "");
                            //放到索引表中备用
    ...
    {
        //直接取解密内容
        var unpackedStrBytes = node.unpackedFileByteArr;
        //导出文本的偏移指针
        var strpos = 0;
        ...
        if (unpackedStrBytes.Length >= 7)//如果总字节长度>=7
        {
            for (var i = 2; i < unpackedStrBytes.Length; i += 5)//以5为单步从第二位开始遍历字节
            {
                ...
                if (unpackedStrBytes.Length - i >= 5)//到最后了就不处理了防止内存越界
                {
                    ...
                    {
                        //取该指示位后面的整数
                        var after1 = BitConverter.ToInt32(unpackedStrBytes, i + 1);
                        if (currentByte == 10)//这个字符是10时
                        {
                            //取指示位前面的整数
                            var before1 = BitConverter.ToInt32(unpackedStrBytes, i - 4);
                        ...
                        {
                            var str = new string[] { "{", currentByte.ToString(), "=`", UnpackSpecialChr(currentByte, after1, 0), "`}\r\n" };
        ...
        //创建一个正好大小的字节数组
        var bytes = new byte[strpos];
        foreach (var pos in arr.Keys)
        ...
        //转换成文本
        var str1 = encoding.GetString(bytes).TrimEnd(new char[1]);
        return str1;
    }
    ...
    private string UnpackSpecialChr(byte curr, int after1, int before1)
    {
        switch (curr)
        {
            ...
            case 10:
                ...
                //字符串类型，字符串数据存在stringtable.bin里
                var str = stringBinMap[after1];

                //<指示符前面的数字::字符串`nstring[指示符前面的数字]`>
                var strArrays = new string[] { "<", before1.ToString(), "::", str, "`", GetNString(str), "`>" };
